 ...
 *
 * Term frequency (TF) and reverse index (RI) keys are both search keys
 * of this format. Field info (FI) keys are as well, but only carry a prefix
 * and field.
 */
var buildSearchKey = function (prefix, field, value, filter, filterKey) {
    return buildKey([prefix, field, value, filter || "", filterKey || ""]);
};
 ...
 */
var buildFieldInfoKey = function (field) {
    return buildKey(["FI", field]);
};
 ...
 *
 * Recommended for keys constructed with buildTermFrequencyKey, buildReverseIndexKey,
 * and buildFieldInfoKey.
 */
